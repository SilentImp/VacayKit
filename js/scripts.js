...
Contacts = (function() {
  function Contacts() {
    ...
    this.form = $('.contacts__form');
    if (this.form.length === 0) {
      return;
    }
    this.success = $('section.contacts.contacts_success');
    this.form.on('submit', this.sendMessage);
  }
...
$(document).ready(function() {
  ...
  ref = $('.best__graph[data-type]');
  results = [];
  for (i = 0, len = ref.length; i < len; i++) {
    graph = ref[i];
    results.push(new Graph($(graph)));
  }
  return results;
});
